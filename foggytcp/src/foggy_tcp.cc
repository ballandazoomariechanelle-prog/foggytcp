...
void* foggy_socket(const foggy_socket_type_t socket_type,
    ...
    // -------------------------------------------------------------------
    // CORRECTION: Initialisation des variables GBN manquantes et du timer.
    // Ces variables sont nécessaires pour la logique dans foggy_backend.cc.
    // -------------------------------------------------------------------
    sock->window.send_base = 0;           // Base de la fenêtre d'envoi
    sock->window.next_seq_num = 0;        // Prochain numéro de séquence à utiliser
    sock->window.effective_window_size = 0; // Calculée plus tard
    sock->window.retransmit_timeout = 0;  // Timer inactif au démarrage
    sock->window.last_send_time.tv_sec = 0;
    sock->window.last_send_time.tv_nsec = 0;
    // -------------------------------------------------------------------
